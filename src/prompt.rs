...
pub(crate) fn build_system_prompt(global_dir: &Path) -> String {
    let cwd = std::env::current_dir().ok();
    build_system_prompt_from(cwd.as_deref(), global_dir)
}

fn build_system_prompt_from(cwd: Option<&Path>, global_dir: &Path) -> String {
...
mod tests {
    ...
    fn prompt_is_coding_focused_and_documents_extension_contract() {
        ...
        let prompt = build_system_prompt_from(Some(&dirs.0), &dirs.0.join("global"));
    ...
    fn global_and_project_agents_instructions_are_injected_in_order() {
        ...
        let prompt = build_system_prompt_from(Some(&project_dir), &global_dir);
